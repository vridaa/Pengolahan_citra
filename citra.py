...
class VideoProcessor(VideoProcessorBase):
    ...
    def recv(self, frame):
        ...
        # Process the image based on the selected method
        if self.method:
            processed_frame = process_image(img_rgb, self.method)
        else:
            processed_frame = img_rgb

        return av.VideoFrame.from_ndarray(processed_frame, format="rgb24")


def display_camera():
    """Displays real-time webcam feed with dynamic processing options."""
    # Ensure session state is initialized for the camera
    if 'run_camera' not in st.session_state:
        st.session_state['run_camera'] = False  # Initialize camera state

    # Create layout columns for displaying original and processed frames
    col1, col2 = st.columns(2)
    ...
    # Select the category and method
    ...
    # Update parameters dynamically based on selected method
    update_parameters(method)
    ...
    if ctx.video_processor:
        # Dynamically assign the method to the VideoProcessor
        ctx.video_processor.method = method

    # Display histograms and frames
    ...
    while st.session_state['run_camera']:
        if ctx.video_processor:
            # Show original and processed frame histograms
            stframe_original.text("Gambar Asli (Real-Time)")
            stframe_hist_original.pyplot(plot_histogram(np.zeros((256, 256)), "Placeholder"))
            stframe_processed.text(f"Gambar Setelah Diolah ({method})")
            stframe_hist_processed.pyplot(plot_histogram(np.zeros((256, 256)), "Placeholder"))
        
        # Add delay to reduce computational load
        time.sleep(0.1)

    # Ensure proper release when stopped
    stframe_original.text("Webcam Dimatikan.")
